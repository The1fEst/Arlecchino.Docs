...
a delete or a decision about color — [Migrating to 2.0](migrating-to-2.0.md) is the edit list, and the
...
## The framework's own colors are the default
...
- `FrameThread.DiscardPending()` drops work that was posted and can no longer run — which is what
  ...
  the next.
- `ThemePalette.Basic`, the sixteen plain colors that were the default before this release.
