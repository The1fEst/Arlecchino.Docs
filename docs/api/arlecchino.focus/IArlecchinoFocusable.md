...
| [`MoveFocus(FocusDirection)`](#movefocus-focusdirection) | Moves the focus one step inside this element, for something that holds focusable parts of its own. Answering `false` hands the step back to the surrounding ring, as the default does. |
