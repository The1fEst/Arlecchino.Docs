...
A map the undo stack never sees: what a scan found per folder, the sizes worked out so far, the state of each connection. It notifies and asks for a frame exactly as a [`TrackedAtomsMap`](../arlecchino.atoms.tracked/TrackedAtomsMap-2.md) does.
