...
| Member | Summary |
|---|---|
| [`Content`](#content) | Draws the content. The region it is handed is as tall as [`ScrollPane.ContentHeight`](../arlecchino.widgets.lists/ScrollPane.md#contentheight) and moved up by the offset, so the delegate always writes at row zero for the first line and never has to know where the window is. |
...
Draws the content. The region it is handed is as tall as [`ScrollPane.ContentHeight`](../arlecchino.widgets.lists/ScrollPane.md#contentheight) and moved up by the offset, so the delegate always writes at row zero for the first line and never has to know where the window is.
