...
| Member | Summary |
|---|---|
| [`Clear()`](#clear) | Forgets the picture, leaving the region to whatever draws next. What the terminal was handed as pixels is undrawn on the next frame — see how [`Surface.Passthrough`](../arlecchino.rendering/Surface.md#passthrough-int-int-string-string) takes a payload back — so this needs no more than forgetting them. |
| [`Draw(SurfaceRegion)`](#draw-surfaceregion) | Draws the picture as large as it goes inside the region without stretching it, centered, and returns an empty region: a picture fills what it is given, so hand over the pane it belongs in. |
...
Forgets the picture, leaving the region to whatever draws next. What the terminal was handed as pixels is undrawn on the next frame — see how [`Surface.Passthrough`](../arlecchino.rendering/Surface.md#passthrough-int-int-string-string) takes a payload back — so this needs no more than forgetting them.
