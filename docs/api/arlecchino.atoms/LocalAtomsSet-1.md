...
A set the undo history never sees: the rows expanded, the folders already walked, the hosts that answered. It notifies and asks for a frame exactly as a [`TrackedAtomsSet`](../arlecchino.atoms/TrackedAtomsSet-1.md) does.

```csharp
public sealed class LocalAtomsSet<T> : AtomsSet<T>, IReadableAtom<IReadOnlySet<T>>
```

**Inherits from** [`AtomsSet`](../arlecchino.atoms/AtomsSet-1.md)&lt;`T`&gt;  
