...
| [`GetEnumerator()`](#getenumerator) | Walks what is waiting, front first, so `foreach` over the queue itself reads the way it does over a list. Reach for [`AtomsQueue.Value`](../arlecchino.atoms/AtomsQueue-1.md#value) where a sequence is what is wanted, LINQ included. |
...
Walks what is waiting, front first, so `foreach` over the queue itself reads the way it does over a list. Reach for [`AtomsQueue.Value`](../arlecchino.atoms/AtomsQueue-1.md#value) where a sequence is what is wanted, LINQ included.
