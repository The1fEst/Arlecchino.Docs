...
| `comparer` | `IEqualityComparer<T>`&lt;`T`&gt; | How [`AtomsList.Remove`](../arlecchino.atoms/AtomsList-1.md#remove-t) finds an item and how a write to the indexer decides it changed nothing; the default comparer for `T` is used when omitted. |
...
Puts several items at the end at once. One notification, one frame and one undo step for the lot, which is what a loop of [`AtomsList.Add`](../arlecchino.atoms/AtomsList-1.md#add-t) cannot give — that would undo a page of rows one row at a time.
