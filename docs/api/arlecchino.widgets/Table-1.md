...
Rows in aligned columns, with a heading and optional sorting. Selection and scrolling are a list box underneath, so a table behaves exactly like a list that happens to draw more per row. Sorting reorders a copy, leaving whatever was assigned to [`Table.Rows`](../arlecchino.widgets/Table-1.md#rows) untouched.
...
| [`Handle(ConsoleKeyInfo)`](#handle-consolekeyinfo) | Moves the selection or confirms it. Sorting is not bound to a key; call [`Table.SortBy`](../arlecchino.widgets/Table-1.md#sortby-int). |
...
**Type** `IReadOnlyList<T>`&lt;[`TableColumn`](../arlecchino.widgets/TableColumn-1.md)&lt;`T`&gt;&gt;
...
Moves the selection or confirms it. Sorting is not bound to a key; call [`Table.SortBy`](../arlecchino.widgets/Table-1.md#sortby-int).
