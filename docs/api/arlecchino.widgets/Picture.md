...
| [`Protocol`](#protocol) | How the picture reaches the terminal. The application's own setting — [`Glyphs.Picture`](../arlecchino.rendering/Glyphs.md#picture) — when left alone, so one pane can differ without every other one being told. |
...
How the picture reaches the terminal. The application's own setting — [`Glyphs.Picture`](../arlecchino.rendering/Glyphs.md#picture) — when left alone, so one pane can differ without every other one being told.

**Type** `Nullable<T>`&lt;[`ImageProtocol`](../arlecchino.rendering/ImageProtocol.md)&gt;
...
| Name | Type | Description |
|---|---|---|
| `pixels` | `ReadOnlySpan<T>`&lt;[`Rgb`](../arlecchino.rendering/Rgb.md)&gt; | The pixels, `width` × `height` of them. |
