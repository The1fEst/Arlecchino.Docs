...
A style built from exact colours. Use it where the colour itself is the point — a swatch, a chart, syntax highlighting — and keep chrome on [`Theme`](../arlecchino.rendering/Theme.md), which follows the terminal theme. Falls back to the nearest palette colour when the terminal cannot do 24-bit.
...
| Member | Summary |
|---|---|
| [`ToString()`](#tostring) | Returns [`RgbTermColor.Ansi`](../arlecchino.rendering/RgbTermColor.md#ansi). |
...
Returns [`RgbTermColor.Ansi`](../arlecchino.rendering/RgbTermColor.md#ansi).
