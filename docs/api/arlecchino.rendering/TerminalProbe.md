...
| Member | Summary |
|---|---|
| [`Ask(IArlecchinoTerminal, TimeSpan)`](#ask-iarlecchinoterminal-timespan) | Asks, waits no longer than it is told, and installs what came back into [`TerminalCapabilities`](../arlecchino.rendering/TerminalCapabilities.md) and [`Glyphs`](../arlecchino.rendering/Glyphs.md). Anything the terminal did not answer is left alone. Call it before the mouse and paste modes go on, so their reports cannot arrive among the answers. |
...
Asks, waits no longer than it is told, and installs what came back into [`TerminalCapabilities`](../arlecchino.rendering/TerminalCapabilities.md) and [`Glyphs`](../arlecchino.rendering/Glyphs.md). Anything the terminal did not answer is left alone. Call it before the mouse and paste modes go on, so their reports cannot arrive among the answers.
...
**Returns** `bool` — `true` when the terminal said anything at all. What it said is in [`TerminalCapabilities.Sixel`](../arlecchino.rendering/TerminalCapabilities.md#sixel), [`TerminalCapabilities.Kitty`](../arlecchino.rendering/TerminalCapabilities.md#kitty) and [`Glyphs.CellWidth`](../arlecchino.rendering/Glyphs.md#cellwidth); a `false` here is how you tell a terminal that cannot draw pictures from one that never replied.
