...
| [`Kitty`](#kitty) | Whether the terminal answered the kitty graphics query. Set by [`TerminalProbe.Ask`](../arlecchino.rendering/TerminalProbe.md#ask-iarlecchinoterminal-timespan); assign it to answer for a terminal that will not. |
| [`Sixel`](#sixel) | Whether the terminal said it speaks sixel. Set by [`TerminalProbe.Ask`](../arlecchino.rendering/TerminalProbe.md#ask-iarlecchinoterminal-timespan); assign it to answer for a terminal that will not, and read it to decide what to offer in a settings screen. |
...
| [`Resolve(ImageProtocol)`](#resolve-imageprotocol) | Turns [`ImageProtocol.Auto`](../arlecchino.rendering/ImageProtocol.md) into the best of what the terminal admitted to, and hands anything else back unchanged. Kitty first: it carries exact colour and lets the terminal do the scaling, where sixel takes a palette of 256 and a guess at the size of a cell. With nothing detected this answers [`ImageProtocol.Blocks`](../arlecchino.rendering/ImageProtocol.md), which is why a picture still appears on a terminal that never replied. |
...
**Type** `Nullable<T>`&lt;[`Rgb`](../arlecchino.rendering/Rgb.md)&gt;
...
Whether [`Glyphs.CellWidth`](../arlecchino.rendering/Glyphs.md#cellwidth) and [`Glyphs.CellHeight`](../arlecchino.rendering/Glyphs.md#cellheight) came from the terminal rather than from the standing guess. Sixel sizing rests on them, so this is how an application tells a picture that will land exactly from one that will land approximately — and it is the only way to tell a terminal that reported ten by twenty from one that said nothing.
...
Whether the terminal answered the kitty graphics query. Set by [`TerminalProbe.Ask`](../arlecchino.rendering/TerminalProbe.md#ask-iarlecchinoterminal-timespan); assign it to answer for a terminal that will not.
...
Whether the terminal said it speaks sixel. Set by [`TerminalProbe.Ask`](../arlecchino.rendering/TerminalProbe.md#ask-iarlecchinoterminal-timespan); assign it to answer for a terminal that will not, and read it to decide what to offer in a settings screen.
...
| Name | Type | Description |
|---|---|---|
| `protocol` | [`ImageProtocol`](../arlecchino.rendering/ImageProtocol.md) | What was asked for. |

**Returns** [`ImageProtocol`](../arlecchino.rendering/ImageProtocol.md) — What to actually draw with.
