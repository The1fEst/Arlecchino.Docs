...
One key press, as the framework hands it to a view. It is `ConsoleKeyInfo` with room for one more modifier: that type stores Shift, Alt and Control as three booleans and has nowhere to put Command, so a terminal that reports Command had to be either misread as an unmodified key or dropped. Everything a view is given goes through here instead.
...
| [`KeyPress(ConsoleKey, KeyModifiers, char)`](#keypress-consolekey-keymodifiers-char) | One key press, as the framework hands it to a view. It is `ConsoleKeyInfo` with room for one more modifier: that type stores Shift, Alt and Control as three booleans and has nowhere to put Command, so a terminal that reports Command had to be either misread as an unmodified key or dropped. Everything a view is given goes through here instead. |
